...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (multipleTables) {
            result = table1.select(table2, columns, conditions);
        } else {
            result = table1.select(columns, conditions);
        }
        return result;
    }
    ...
    Condition condition(Table... tables) {
        ...
        for (Table table : tables) {
            if (table.findColumn(next) != -1) {
                String col2 = columnName();
                return new Condition(new Column(col1, tables),
                    relation, new Column(col2, tables));
            }
        }
        String val2 = literal();
        return new Condition(new Column(col1, tables), relation, val2);
    }
